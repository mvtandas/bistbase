...
import { backfillSignalOutcomes } from "@/lib/cron/backfill-signal-outcomes";
...
export async function POST(request: NextRequest) {
  ...
  try {
    cronLog = await prisma.cronLog.create({
      data: { cronName: "backfill-signal-outcomes", status: "RUNNING" },
    });
    ...
    console.error("Backfill signal outcomes failed:", error);
